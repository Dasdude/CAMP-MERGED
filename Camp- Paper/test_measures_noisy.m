function [err_mat,loss_mat] = test_measures_noisy(loss_handle_list,param_ref,make_dist_handle,per_samples_list,total_trials,total_samples_x)
...
%             f_ref = make_dist_handle(rand(1,2)*5+1);
            f_noise = make_dist_handle([per_samples,.1]);
            x = random(f_ref,[total_samples,1]);
            x = x+random(f_noise,[total_samples,1]);
    ...
    %         x = dbm2linear(double(int64(linear2dbm(x))));
            ...
            for loss_idx = 1:length(loss_handle_list)
                ...
                [params,loss_val,~,out]=fmincon(fun,param_init,[],[],[],[],[-inf,eps],[],[],options);
