function [err_mat,loss_mat] = test_measures_low_precision(loss_handle_list,param_ref,make_dist_handle,per_samples_list,total_trials,total_samples_x)
...
%             f_ref = make_dist_handle(rand(1,2)*5+1);
    ...
    %         x = dbm2linear(double(int64(linear2dbm(x))));
            x_prctile = prctile(x,(1:per_samples)./(per_samples+1)*100);
            x_prctile = [-inf,x_prctile,inf];
            for prctile_idx = 2:length(x_prctile)
                x(x<=x_prctile(prctile_idx)&x>x_prctile(prctile_idx-1))=x_prctile(prctile_idx-1);
            end
            tr = icdf(f_ref,0);
