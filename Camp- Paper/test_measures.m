function [err_mat,param_mat,guess_mat,loss_mat] = test_measures(loss_handle_list,param_ref,make_dist_handle,per_samples_list,total_trials,total_samples_list)
...
guess_mat = zeros(length(total_samples_list),length(per_samples_list),length(loss_handle_list),total_trials)*nan;
loss_mat = zeros(length(total_samples_list),length(per_samples_list),length(loss_handle_list),total_trials)*nan;
...
%             f_ref = make_dist_handle(rand(1,2)*5+1);
    ...
    %         x = dbm2linear(double(int64(linear2dbm(x))));
            ...
            for loss_idx = 1:length(loss_handle_list)
                ...
                fun = @(theta)loss_fun(x_trunc',per,[theta,param_ref(2)]);
                if per==1
                    continue
                end
                
                options = optimoptions(@fmincon,'Display','off','Algorithm','interior-point','MaxFunctionEvaluations',10000);
                [params,loss_val,~,out]=fmincon(fun,param_init(1),[],[],[],[],[eps],[],[],options);
