...
[err_mat,loss_mat] = test_measures_low_precision(loss_handle_list,ref_param,make_dist_handle,per_list,total_trials,total_samples);
...
figure;plot(per_list,err_mat_mean_x_per');legend(n_str);grid on;xlabel('PER');ylabel(' $E_\theta := ||\hat{\theta}-\theta||$');title(sprintf('Full Distribution Total Samples %d, total trials: %d',total_samples,total_trials));saveas(gcf,sprintf('./Plots/Paper Plots/BME_Res Preceiossn %d %d %s.png',total_samples(1),total_trials,dist_name));
...
figure;plot(per_list,-loss_mat_mean_per');legend(n_str,'location','best');grid on;xlabel('PER');ylabel('Average Log Likelihood');title(sprintf('Average of loglikelihood (Training Total Samples %d, Total Trials: %d)',total_samples,total_trials));saveas(gcf,sprintf('./Plots/Paper Plots/BME_Res_LL PReceison%d %d %s.png',total_samples(1),total_trials,dist_name));
figure;
bin_edges = 0:1:50;
