...
if merge ==2
    data_merged_fixed = readtable('Dataset/all_merged.csv','ReadVariableNames',true);
    data_merged_fixed = clean_intersection_data(data_merged_fixed);
    writetable(data_merged_fixed,'Dataset/all_merged_cleaned.csv');
    ...
    per_roadmap_heatmap(data_merged_fixed,20,10,'./Plots/DataAnalysis/All/');
    per_roadmap_heatmap(dif_leg_nlos_data,20,10,'./Plots/DataAnalysis/NLOS/');
    per_roadmap_heatmap(dif_leg_los,20,10,'./Plots/DataAnalysis/LOS/');
    per_roadmap_heatmap(same_leg_data,20,10,'./Plots/DataAnalysis/SAME/');
%     plot_roadmap_transreciev(data_merged_fixed,10000,'All',1);
%     close all
%     plot_roadmap_transreciev(dif_leg_nlos_data,10000,'NLOS',0);
%     close all;
%     plot_roadmap_transreciev(dif_leg_los_data,10000,'LOS',0);
%     close all;
%     plot_roadmap_transreciev(same_leg_data,1000,'SameLeg',0);

    writetable(same_leg_data,'Dataset/same_leg.csv');
    writetable(dif_leg_los_data,'Dataset/dif_leg_los.csv');
    writetable(dif_leg_nlos_data, 'Dataset/dif_leg_nlos.csv');
end