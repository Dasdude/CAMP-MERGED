...
%         fading_params(fading_params(:,2)>prctile(fading_params(:,2),max_val_prctile),2)=prctile(fading_params(:,2),max_val_prctile);
        fading_params(:,1)=medfilt1(fading_params(:,1),20);
        fading_params(:,2)=medfilt1(fading_params(:,2),20);
%         param_index = 180;
...
%         pathloss = 1.3*(20*log10(1:d_max)+20*log10(CARRIER_FREQ)+20*log10(4*pi/LIGHT_SPEED));
        ...
        figure;plot(TX_POWER-pathloss);hold on;plot(data_dbm_mean);
        %% Percentile Plot
        ...
        figure;plot(TX_POWER-pathloss);hold on;plot(percentiles_rssi_per_inc(:,4));
