...
mkdir(plot_folder);
pl_sudo_val = -110
long_lim = [min(data.rx_long),max(data.rx_long)];

lat_lim = [min(data.rx_lat),max(data.rx_lat)];
lat_lim = [33.755,33.764];
long_lim = [-117.996,-117.984];
lat_flag = data.lat>lat_lim(1)&data.lat<lat_lim(2)&data.rx_lat>lat_lim(1)&data.rx_lat<lat_lim(2);
long_flag = data.long>long_lim(1)&data.long<long_lim(2)&data.rx_long>long_lim(1)&data.rx_long<long_lim(2);
inboundry_flag = lat_flag&long_flag;
data = data(inboundry_flag,:);
data.RSS(data.RSS<=pl_sudo_val)=pl_sudo_val;
lat_step = 1e-4
long_step = 4e-4
data.lat = data.lat- mod(data.lat,lat_step);
data.rx_lat = data.rx_lat-mod(data.rx_lat,lat_step);
data.long = data.long - mod(data.long,long_step);
data.rx_long = data.rx_long - mod(data.rx_long,long_step);
...
for dist_iter = 50:dist_step:500
    ...
    while height(data_dist_fixed)>0
        lat_lon_pair =[data_dist_fixed.lat,data_dist_fixed.long,data_dist_fixed.rx_lat,data_dist_fixed.rx_long];
        lat_lon_pair2 = [data_dist_fixed.rx_lat,data_dist_fixed.rx_long,data_dist_fixed.lat,data_dist_fixed.long];
