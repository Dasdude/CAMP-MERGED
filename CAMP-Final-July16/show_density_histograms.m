...
close all
% clear
folder_path = fullfile('Plots','Data Analysis','Veh Density Including Lost Packets');
mkdir(folder_path);
if ~exist('dens_east')
    [field_same_low_dirty,field_same_low_dirty_mat,dens_east_same_low,dens_west_same_low,dens_same_low] = load_field_data_scenario('same','low',800);
    [field_same_med_dirty,field_same_med_dirty_mat,dens_east_same_med,dens_west_same_med,dens_same_med] = load_field_data_scenario('same','med',800);
    [field_same_high_dirty,field_same_high_dirty_mat,dens_east_same_high,dens_west_same_high,dens_same_high] = load_field_data_scenario('same','high',800);

    [field_opposite_low_dirty,field_opposite_low_dirty_mat,dens_east_op_low,dens_west_op_low,~] = load_field_data_scenario('opposite','low',800);
    [field_opposite_med_dirty,field_opposite_med_dirty_mat,dens_east_op_med,dens_west_op_med,~] = load_field_data_scenario('opposite','med',800);
    [field_opposite_high_dirty,field_opposite_high_dirty_mat,dens_east_op_high,dens_west_op_high,~] = load_field_data_scenario('opposite','high',800);
...
figure('Position',[1,1,2000,1000]);histogram(table2array(dens_op));title('Encounter Scenario Agg Veh Density (V/M/L)');saveas(gcf,fullfile(folder_path,'EncounerAgg.png')); 
figure('Position',[1,1,2000,1000]);histogram(table2array(dens_same));title('Following Scenario Agg Veh Density (V/M/L)');saveas(gcf,fullfile(folder_path,'FollowingAgg.png')); 
...
figure('Position',[1,1,2000,1000]);histogram(mean(table2array(dens_op),2));title('Encounter Scenario Average Veh Density (V/M/L)');saveas(gcf,fullfile(folder_path,'EncounerAverage.png')); 
figure('Position',[1,1,2000,1000]);histogram(mean(table2array(dens_same),2));title('Following Scenario Average Veh Density (V/M/L)');saveas(gcf,fullfile(folder_path,'FollowingAverage.png')); 
...
figure('Position',[1,1,2000,1000]);histogram(mean(table2array(dens_same_low(:,1:6)),2),'BinEdges',[0:60]);title('Following Low Density Scenario Average Veh Density (V/M/L) No HOV');ylim([0 3e5]);xlim([0,60]);
...
figure('Position',[1,1,2000,1000]);histogram(mean(table2array(dens_op_low(:,[1:6,9:end])),2),'BinEdges',[0:60]);title('Encounter Low Density Scenario Average Veh Density (V/M/L) No HOV');ylim([0 3e5]);xlim([0,50]);
...
figure('Position',[1,1,2000,1000]);histogram(mean(table2array(dens_same_low),2),'BinEdges',[0:60]);title('Following Low Density Scenario Average Veh Density (V/M/L) HOV Included');ylim([0 3e5]);xlim([0,50]);
...
function [] = plot_lane_wise(dens,folder_path,bound_name)
    ...
    histogram(values(:));
    title(sprintf('All'));
    suptitle(sprintf('%s Bound Densities(Veh/Mile/Lane)',bound_name,'.png'));
    saveas(gcf,fullfile(folder_path,bound_name))
end
